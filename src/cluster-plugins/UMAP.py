...
#     --pca_batch_size=5 \
#     --parallelize=0 \
#     --parameters='{"ndims":2, "pca_fraction":1, "neighbors":10, "distance":0.1}'
...
def cluster_parameters():
    return [
          ...
          ["neighbors",    "neighbors",    "",        "10",  1, [], lambda n,M,V,C: positive_callback("neighbors",n,M,V,C),    True],
          ["distance",     "distance",     "",        "0.1", 1, [], lambda n,M,V,C: positive_callback("distance",n,M,V,C),     True],
    ]

def do_cluster(activations_tocluster, ilayer, layers, parameters):
    if ilayer in layers:
        print("reducing dimensionality of layer "+str(ilayer)+" with UMAP...")
        fit = UMAP(verbose = 3,
                   n_components = int(parameters["ndims"]),
                   n_neighbors = int(parameters["neighbors"]),
                   min_dist = float(parameters["distance"])
