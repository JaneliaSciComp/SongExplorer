...
def model_parameters(time_units, freq_units, time_scale, freq_scale):
    return [
        ...
        ["nconvlayers",     "# conv layers",           '',                   '2',                    1, [],                 None,                                              True],
        ...
        ["dropout_loc",     "dropout loc",             ['none',
                                                        'input',
                                                        'bottleneck'],       'none',                 1, [],                 None,                                                          True],
...
def create_model(model_settings, model_parameters, io=sys.stdout):
  ...
  dropout_loc = model_parameters['dropout_loc']
  ...
  if representation == "waveform":
    ...
    decoder_layers.append(Reshape((ninput_tics,model_settings['audio_nchannels'])))
    ...
    if model_parameters['range'] != "":
      ...
      # decoder_layers.append()
    #x = Slice([0,0,round(x.shape[2]/5),0], [-1,-1,-1,-1])(x)
    #x = Sparsify(70.)(x)
  ...
  if dropout>0 and dropout_loc=="input":
      x = Dropout(dropout)(x)
  ...
  while x_shape[1] >= dilated_kernel_size[0] and \
    ...
    decoder_layers.append(Conv2DTranspose(1 if first_conv else nfeatures[0], kernel_sizes[0],
  ...
  while noutput_tics >= dilated_kernel_size and \
    ...
    decoder_layers.append(Conv2DTranspose(1 if first_conv else nfeatures[1], (kernel_sizes[1], x_shape[2]),
                                          strides=strides, dilation_rate=[dilation_rate[0], 1]))
    first_conv = False
  ...
  if dropout>0 and dropout_loc=="bottleneck":
      x = Dropout(dropout)(x)
      decoder_layers.append(Dropout(dropout))

  if pool_kind:
    x = pool_kind(pool_size=pool_size, strides=pool_size)(x)
    decoder_layers.append(UpSampling2D(size=pool_size))
    x_shape = x.shape
    noutput_tics = math.floor(noutput_tics / pool_size[0])

  # final dense layers (or actually, pan-freq pan-time 2D convs)
  for idense, nunits in enumerate(denselayers):
    x = Conv2D(nunits, (noutput_tics if idense==0 else 1, x_shape[2]))(x)
    decoder_layers.append(Conv2DTranspose(x_shape[3], (noutput_tics if idense==0 else 1, x_shape[2])))
    hidden_layers.append(x)
    x_shape = x.shape
    x = ReLU()(x)
    decoder_layers.append(ReLU())
  ...
  while type(decoder_layers[-1]) in [ReLU, Dropout]:
      decoder_layers.pop()
  while(decoder_layers):
    x = decoder_layers.pop()(x)

  final = x

  #final = Reshape((-1,model_settings['nlabels']))(x)

  print('autoencoder.py version = 0.1', file=io)
  return tf.keras.Model(inputs=inputs, outputs=[hidden_layers, final], name="autoencoder")
