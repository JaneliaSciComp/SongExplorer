...
# ethogram.py <logdir> <model> <check-point> <tffile> <tic_rate>
...
_,logdir,model,check_point,tffile,tic_rate = argv
print('logdir: '+logdir)
print('model: '+model)
print('check_point: '+check_point)
print('tffile: '+tffile)
print('tic_rate: '+tic_rate)
tic_rate_recording=float(tic_rate)
...
thresholds=[]
with open(os.path.join(logdir,'train_'+model,'thresholds.ckpt-'+check_point+'.csv')) as fid:
...
for ithreshold in range(len(precision_recall_ratios)):
  ...
  start_tics = np.round((start_tics[:ifeature] / sample_rate_probabilities \
                         - half_stride_sec) \
                        * tic_rate_recording).astype(np.int)
