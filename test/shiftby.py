...
for isample0 in range(np.shape(fingerprints0['arr_0'])[0]):
  isample512 = fingerprints512['samples'].tolist().index(fingerprints0['samples'][0])
  if not all(fingerprints0['arr_0'][0,:-shiftby,0] == fingerprints512['arr_0'][isample512,shiftby:,0]):
    print("ERROR: traces are not shifted properly")
  if all(fingerprints512['arr_0'][isample512,:-shiftby,0]==0):
