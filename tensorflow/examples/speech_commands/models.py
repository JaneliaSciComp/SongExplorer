...
def create_conv_model(fingerprint_input, model_settings, is_training):
  ...
  (fingerprint_input)
      ...
      [BiasAdd]<-(bias)
        ...
        [Relu]
          v
      [MaxPool]   ??? not sure about this second max-pool...
          v
      [MatMul]<-(weights)
          v
      [BiasAdd]<-(bias)
  ...
  first_filter_height = 16
  first_filter_width = 16
  for iconv in range(len(filter_counts)):
    filter_height = first_filter_height>>iconv
    filter_width = first_filter_width>>iconv
    filter_count_prev = 1 if iconv==0 else filter_counts[iconv-1]
    filter_count = filter_counts[iconv]
    weights = tf.Variable( tf.truncated_normal(
              [filter_height, filter_width, filter_count_prev, filter_count],
              stddev=0.01))
    bias = tf.Variable(tf.zeros([filter_counts[iconv]]))
    if iconv==0:
      inarg = fingerprint_4d
    else:
      inarg = tf.nn.max_pool(dropout, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME')
    tf.logging.info('conv layer %d: in_shape = %s, conv_shape = %s' %
          (iconv, inarg.get_shape(), weights.get_shape()))
    conv = tf.nn.conv2d(inarg, weights, [1, 1, 1, 1], 'SAME') + bias
  ...
  conv_shape = dropout.get_shape()
  conv_output_width = conv_shape[2]
  conv_output_height = conv_shape[1]
  conv_element_count = int(conv_output_width * conv_output_height * filter_counts[-1])
  flattened_conv = tf.reshape(dropout, [-1, conv_element_count])
  label_count = model_settings['label_count']
  final_fc_weights = tf.Variable( tf.truncated_normal(
          [conv_element_count, label_count], stddev=0.01))
