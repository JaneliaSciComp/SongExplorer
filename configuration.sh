...
train_it () {
    if [ "$train_gpu" -eq "1" ] ; then
        generic_it "$1" "$2" "$train_where" \
                   "2 1 1" "" "-n 2 -W 10080 -gpu \"num=1\" -q gpu_rtx"
    else
        generic_it "$1" "$2" "$train_where" \
                   "12 0 1" "" "-n 12 -W 10080"
    fi
...
mistakes_it () {
    generic_it "$1" "$2" "$mistakes_where" "1 0 1" "" "-n 1 -W 60"
}
...
cluster_it () {
    generic_it "$1" "$2" "$cluster_where" "8 0 1" "" "-n 8 -W 60"
}
...
classify_it () {
    ...
    if [ "$classify_gpu" -eq "1" ] ; then
        bsubstdout=`generic_it "$1" "$3-tf.log" "$classify_where" \
                   "2 1 1" "" "-n 2 -W 60 -gpu \"num=1\" -q gpu_rtx"`
    else
        bsubstdout=`generic_it "$1" "$3-tf.log" "$classify_where" \
                   "12 0 1" "" "-n 8 -W 60"`
    fi
    jobid=`echo $bsubstdout | grep -o "[0-9]*"`
